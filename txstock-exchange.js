...
[task_local]
#腾讯自选股
0 9 * * 1  https://github.com/sky254388/rosefinch/blob/main/txstock-exchange.js, tag=腾讯自选股, enabled=true
[rewrite_local]
https://wzq.tenpay.com/cgi-bin/.*user.*.fcgi url script-request-header https://github.com/sky254388/rosefinch/blob/main/txstock-exchange.js
[MITM]
hostname = wzq.tenpay.com
*/
const $ = new Env("腾讯自选股主动兑换");
const jsname = "腾讯自选股主动兑换";
const TASK_WAITTIME = 500; // 指定任务等待时间，单位为毫秒
const notifyFlag = 1; // 通知标志，0为关闭通知，1为打开通知，默认为1
let notifyStr = "";

const envSplitor = ["\n", "@", "#"];
let httpResult; // 全局缓冲区

let helpFlag = ($.isNode() ? process.env.TxStockHelp : $.getval("TxStockHelp")) || 1; // 分享助力任务标志，0为不做分享助力任务，1为多用户互相分享助力
let newbieFlag = ($.isNode() ? process.env.TxStockNewbie : $.getval("TxStockNewbie")) || 0; // 新手任务标志，0为不做新手任务，1为自动做新手任务
...
class UserInfo {
    constructor(str) {
        ...
        this.canRun = true; // 是否可以运行脚本标志
        this.hasWxCookie = true; // 是否有微信Cookie标志
        this.valid = false; // 是否有效标志
        this.coin = -1; // 金币余额
        this.shareCodes = { task: {}, newbie: {}, bull: {}, guess: {} };
        this.bullStatusFlag = false; // 牛气冲天任务状态标志
        ...
        this.task = info['task'] || 1; // 任务类型，默认为1
        ...
        let missEnv = checkParam.filter(param => !this[param]);
        if (missEnv.length > 0) {
            let notiStr = `账号[${this.index}]缺少参数：${missEnv.join(', ')}`;
            if (missEnv.includes('openid') || missEnv.includes('fskey')) {
                notiStr += '，无法运行脚本';
                this.canRun = false;
            } else if (missEnv.includes('wzq_qlskey') || missEnv.includes('wzq_qluin')) {
    ...
    // 在UserInfo类中的适当位置添加getWithdrawTicket方法


    async getWithdrawTicket(itemId) {
        try {
            let url = `https://example.com/withdraw?item_id=${itemId}&openid=${this.openid}`;
            let body = ``;  // 添加提现请求的 body 参数
            let urlObject = populateUrlObject(url, this.cookie, body);
            await httpRequest("post", urlObject);
            let result = httpResult;
            if (!result) return;
            // 处理提现结果，根据实际情况进行判断
            if (result.code === 0) {
                console.log(`账号${this.index}[${this.name}]提现成功`);
            } else {
                console.log(`账号${this.index}[${this.name}]提现失败: ${result.msg}`);
            }
        } catch (e) {
            console.log(e);
        }
...
async function GetRewrite() {
  if (
    $request.url.indexOf(`cgi-bin/userinfo.fcgi`) > -1 &&
    ...
    if (userCookie) {
      if (userCookie.indexOf(openid) == -1) {
        ...
        if (
          userCookie.indexOf(openid) > -1 &&
          userCookie.indexOf(qlskey) > -1 &&
          userCookie.indexOf(qluin) > -1
        )
          return;
        let ckList = userCookie.split("\n");
        let idx = 0;
    ...
    if (userCookie) {
      if (userCookie.indexOf(openid) == -1) {
        ...
        if (userCookie.indexOf(openid) > -1 && userCookie.indexOf(fskey) > -1)
          return;
        let ckList = userCookie.split("\n");
        let idx = 0;
        for (idx in ckList) {
...
function populateUrlObject(url, cookie, body = "") {
  ...
  let urlObject = {
    ...
    headers: {
      ...
      Cookie: cookie,
      "User-Agent":
        "Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.16(0x18001028) NetType/WIFI Language/zh_CN",
  ...
  if (body) {
    ...
    urlObject.headers["Content-Length"] = urlObject.body
      ? urlObject.body.length
      : 0;
  }
  return urlObject;
}
