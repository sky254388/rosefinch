...
*/
const jsname = "中青看点文章视频"; // 设置脚本名称
const $ = Env(jsname); // 创建名为 $ 的变量，并初始化为 Env(jsname)

const notifyFlag = 1; // 0为关闭通知，1为打开通知，默认为1
const logDebug = 0; // 设置调试日志标志，默认为关闭调试日志

let notifyStr = ""; // 初始化通知字符串
...
let userCookieArr = []; // 初始化用户 Cookie 数组
let bodyArr = []; // 初始化请求体数组

let userIdx = 0; // 初始化用户索引

let artCatid = 0; // 初始化文章类别 ID
let videoCatid = 1453; // 初始化视频类别 ID

let readLength = 0; // 初始化阅读长度
let maxReadLength = 16; // 设置最大阅读长度
let rewardCount = []; // 初始化奖励计数数组
let userArtList = []; // 初始化用户文章列表数组

let artPreBody = "p=swNMsLAQgw3E%3D"; // 文章请求体前缀
let timePreBody = "p=swNMsLAQgw3E%3D"; // 时间请求体前缀

let key = "6HPjSZFH"; // 密钥

let fixStr = "jdvylqchJZrfw0o2DgAbsmCGUapF1YChc"; // 修复字符串
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    // 如果没有 $request 对象，执行以下步骤
    if (!(await checkEnv())) {
      // 检查环境是否符合要求，如果不符合，返回
      return;
    }

    await initAccountInfo(); // 初始化账户信息
    // 以下是一系列异步操作
    await userDouble();
    await userGetList();
    await userReads();
...
async function checkEnv() {
  let userCookie =
    ($.isNode() ? process.env.zqkdCookie : $.getdata("zqkdCookie")) || "";
...
async function GetArtsInfo(artItem, idx) {
  ...
  let url =
    `https://kandian.wkandian.com/v5/article/${urlPattern}.json?` + desStr;
...
async function CompleteArt(id, type, idx) {
  ...
  if (result.success == true) {
    if (result.items && result.items.read_score) {
      ...
      console.log(
        `账号${idx + 1}领取${str}奖励：获得${result.items.read_score}金币`
      );
    } else {
      if (result.items && result.items.max_notice) {
        console.log(
          `账号${idx + 1}没有领取到${str}奖励：${result.items.max_notice}`
        );
      } else {
...
async function UpdateStayTime(tsceond, idx) {
  ...
  if (result.success == true) {
    console.log(
      `账号${idx + 1}更新时长${tsceond}秒成功，今日阅读时长：${result.time}秒`
    );
  } else {
...
async function GetTaskList(idx) {
  let caller = printCaller();
  let url =
    "https://kandian.wkandian.com/v17/NewTask/getTaskList.json?app_version=3.9.8&" +
    userCookieArr[idx];
  let urlObject = populateGetUrl(url);
  await httpGet(urlObject, caller);
  let result = httpResult;
  ...
  if (result.success == true) {
    if (
      result.items &&
      result.items.task_promotion &&
      result.items.task_promotion.is_double == 0
    )
      await GetReadDouble(idx);
  } else {
...
async function GetReadDouble(idx) {
  let caller = printCaller();
  let url =
    "https://kandian.wkandian.com/v17/NewTask/setJoinTaskPromotion.json?" +
    userCookieArr[idx];
  let urlObject = populateGetUrl(url);
  await httpGet(urlObject, caller);
  let result = httpResult;
