...
*/
let targetTime = 8000; //要刷的目标时长，单位为毫秒
let step = 360; //每次刷的时长，单位为毫秒
let taskNum = 10; //领的宝箱总数

const jsname = "中青看点听歌";
const $ = Env(jsname); //创建了一个新的Env对象，这个对象将被用于加载和管理脚本的配置和环境信息
const notifyFlag = 1; //0为关闭通知，1为打开通知,默认为1
const logDebug = 0; //0 为关闭日志调试，1 为打开日志调试，默认为 0

//const notify = $.isNode() ? require('./sendNotify') : '';
let notifyStr = ""; //用于存储通知信息

let rndtime = ""; //毫秒，用于存储随机时间
let httpResult; //global buffer，空的 HTTP 响应结果变量，用于存储从服务器获取的响应

var CryptoJS = require("crypto-js"); //使用 Node.js 的 require() 函数来引入 crypto-js 库，该库提供了一些加密和解密的功能

let userCookieArr = []; //用于存储用户的 Cookie
let bodyArr = [];

let userIdx = 0; //整数变量，初始值为 0，用于记录用户的索引
let compCount = 0; //整数变量，初始值为 0，用于记录完成的任务数

let rewardCount = []; //用于存储奖励的数量
let timeRecord = []; //用于存储时间记录
let timeUpdateFlag = []; //用于存储时间更新标志
...
!(async () => {
  ...
  if (typeof $request !== "undefined") {
    // 如果变量 $request 已定义，使用 jQuery 的 msg() 方法显示一条消息
    $.msg(jsname + ": 此脚本不做重写，请检查重写设置");
  } else {
    // 如果变量 $request 未定义，执行以下操作
    if (!(await checkEnv())) {
      // 使用 await 关键字等待 checkEnv() 函数的返回值，并检查返回值是否为 false
      return;
    }
...
async function SetCompleteSec(tsecond, idx) {
  ...
  let reqBody =
    `access=4G&app_name=zqkd_app&app_version=3.9.8&version_code=67&sec=${tsecond}&` +
    userCookieArr[idx];
  ...
  if (result.success == true) {
    if (result.items) {
      console.log(
        `账号${idx + 1}更新听歌时长成功：今日听歌时长${result.items}秒`
      );
...
async function GetReward(body, idx, uIdx) {
  ...
  if (result.success == true) {
    console.log(
      `用户${uIdx + 1}领取听歌任务${idx + 1}奖励成功，获得${
        result.items.score
      }金币`
    );
    ...
    console.log(
      `用户${uIdx + 1}领取听歌任务${idx + 1}奖励失败：${result.message}`
    );
  }
}
...
async function GetDouble(body, idx, uIdx) {
  ...
  if (result.success == true) {
    console.log(
      `用户${uIdx + 1}领取听歌任务${idx + 1}翻倍奖励成功，获得${
        result.items.score
      }金币`
    );
    rewardCount[uIdx] += parseInt(result.items.score);
    ...
    console.log(
      `用户${uIdx + 1}领取听歌任务${idx + 1}翻倍奖励失败：${result.message}`
    );
  }
}
