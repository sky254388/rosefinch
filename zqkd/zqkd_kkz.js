...
async function checkEnv() {
  ...
  console.log(
    `共找到${userCookieArr.length}个用户，本次运行将查询${idRunList.length}个看看赚任务ID`
  );
  return true;
}
...
async function encodeUserBody(idx, task_id) {
  replacedStr =
    bodyTemplate + `&task_id=${task_id}&` + userCookieArr[idx] + fakeSign;
...
async function adlickstart(body, id, idx) {
  ...
  if (result.success == true) {
    ...
    if (result.error_code == 200001)
      console.log(`非法请求，请检查用户${idx + 1}的ck`);
    return false;
  }
...
async function adlickend(body, idx) {
  ...
  if (result.success == true) {
    console.log(
      `用户${idx + 1}完成看看赚[id:${result.items.banner_id}]任务，获得${
        result.items.score
      }金币`
    );
...
async function getBoxRewardConf() {
  ...
  let url =
    "http://kandian.wkandian.com/WebApi/Nameless/getBoxRewardConf?" + reqCk;
  let urlObject = populateGetUrl(url);
  urlObject.headers.Referer =
    "http://kandian.wkandian.com/h5/20190527watchMoney/?" + reqCk;
...
async function getBoxReward(id) {
  ...
  if (result.status == 1) {
    console.log(
      `用户${userIdx + 1}打开第${id + 1}个看看赚宝箱获得：${result.data}金币`
    );
    rewardCount[userIdx] += parseInt(result.data);
  } else {
    console.log(
      `用户${userIdx + 1}领取第${id + 1}个看看赚宝箱失败：${result.msg}`
    );
  }
}
////////////////////////////////////////////////////////////////////
