...
================Loon==============
[Script]
cron "1-59/15 6-23 * * *" script-path=  ,tag=今日头条极速版
...
let userHeaderArr = [];
let jrttjsbFarm =
  ($.isNode() ? process.env.jrttjsbFarm : $.getdata("jrttjsbFarm")) || 1;
...
async function ListArts() {
    ...
    await httpGet(urlObject, caller)
      .catch((error) => {
        console.log(`获取阅读列表失败：${error.message}`);
        return;
      });
  ...
  if (result.err_no == 0) {
    ...
    if (result.data.sleeping == true) {
      if (sleepHour >= 12) {
        ...
        console.log(
          `用户${userIdx + 1}睡眠中，已经睡了${sleepHour}小时，可以获得${
            result.data.sleep_unexchanged_score
...
async function RewardFarmTask(id) {
  ...
  if (result.status_code == 0) {
    ...
    console.log(
      `用户${userIdx + 1}领取农场任务奖励[task_id=${result.data.task_id}]获得${
...
async function RewardTreeThreeGift(task_id) {
  ...
  if (result.status_code == 0) {
    console.log(
      `用户${userIdx + 1}领取种树三餐礼包获得${result.data.reward_item.num}${
        result.data.reward_item.name
      }`
    );
  } else {
